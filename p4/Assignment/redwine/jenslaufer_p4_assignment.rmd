...
df %>%
  ggplot(mapping=aes(x=quality, y=sweetness)) +
  geom_point(alpha=0.2, position = 'jitter', size=2) +
  scale_x_continuous(breaks=seq(0, 10, 1)) 
```
...
df %>% 
  ...
  mutate(num_quality=n()) %>%
  group_by(quality_bucket, sweetness, num_quality) %>%
  summarise(num=n()) %>%
  mutate(pct=num/num_quality * 100) %>%
  ggplot(mapping=aes(x=quality_bucket, y=pct, fill=sweetness)) +
     geom_bar(stat='identity') +
     geom_text(mapping=aes(label=paste(round(pct,1),"%",sep="")),
               position=position_stack(vjust=0.5)) +
...
df[, +which(names(df) %in% c('density','fixed.acidity',
        'citric.acid', 'residual.sugar', 'alcohol'))] %>%
  gather(-density, key = "var", value = "value")  %>%
  ggplot(aes(x = density, y=value)) +
    geom_point(alpha=0.1, color='black', size=3) +
    geom_smooth(method = 'lm') +
    facet_wrap(~var, scales = "free") 
```

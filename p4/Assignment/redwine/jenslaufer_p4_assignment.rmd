...
```
There is moderate a downhill correlation between the quality and volatile acidity. The better the quality of a wine the less volatile acidity it has. The volatile acidity is mostly vinegar acid, which can be tasted as a wine fault.
...
There are for 4 wines which exceeds the limit for volatile acidity. These wines are rated low and average. This also supports the thesis that too much volatile acidity can be tasted.
...
It seems that the concentration of total sulphur dioxide does not have such a big  influence on wine quality. From the 7 wines which exceed the maximum total sulphur dioxide concentration are even 2 high quality wines . It can also be seen from the boxplot overlay that the median for all kind of quality factors is more or less the same.


### Correlation fixed.acidity, volatile.acidity, citric.acidity, pH


```{r, echo=FALSE, fig.height=20, fig.width=20, message=FALSE, warning=FALSE}

ggpairs(df, columns = c('fixed.acidity', 'volatile.acidity', 'citric.acid', 'pH'))
  
```

### Correlation between sulphates, volatile.acidity and alcohol

Alcohol, volatile acidity and sulphates are the variables that have the most influence on wine quality. Therefore they are candidates to include them into a multiple linear regression model. It's important to check if these features are independent of each other, to avoid colinearity. 
...
From the scatterplots and the correlation matrix it could be found out, that alcohol, 
volatile acidity and sulphates are the attributes which have the most influence 
...
df %>%
  ...
  ggplot(mapping=aes(x=volatile.acidity, y=alcohol)) +
  geom_point(alpha=0.8, mapping=aes(fill=quality_bucket), size=5, shape=21) +
...
df %>%
  ...
  geom_point(alpha=0.7, mapping=aes(fill=quality_bucket), size=5, shape=21) +
  scale_fill_brewer(palette ='RdYlGn') +
  scale_x_log10()
...
```{r, echo=FALSE, message=FALSE, warning=FALSE,
df %>% 
  ggplot(mapping=aes(x=quality_factor, y=alcohol)) +
    geom_point(alpha=0.1, position = 'jitter' ,mapping=aes(size=volatile.acidity))
```

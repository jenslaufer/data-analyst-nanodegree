...
listings_df$instant_bookable = as.logical(listings_df$instant_bookable == "true")
listings_df$is_family_preferred = as.logical(listings_df$is_family_preferred == "true")
...
```{r echo=FALSE, warning=FALSE, message=FALSE, Creation_of_high_level_dataframe}
 ...
 nightly_revenue <- function(available, price) {
 ...
 revenue_per_available_room <- function(available, revenue) {
    revenue / length(available)
 }
 
 yearly_revenue <- function(available, revenue) {
    365 * revenue / length(available)
 }
 

listings_df <- calendar_df %>% 
          mutate(nightly_revenue=nightly_revenue(available, price.local_price)) %>% 
          group_by(id) %>% 
          summarize(currency=first(price.local_currency),
                    occupancy=occupancy(available),
                    average_daily_rate=average_daily_rate(available, sum(nightly_revenue)),
                    revenue_per_available_room=revenue_per_available_room(available, sum(nightly_revenue)),
                    yearly_revenue=yearly_revenue(available, sum(nightly_revenue)),
                    revenue=sum(nightly_revenue)) %>%

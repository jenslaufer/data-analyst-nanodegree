...
ggsave(file="yearly_revenue_by_revenue_count.png", yr_pc_pl)
yr_pc_pl
```
...
ggsave(file="occupancy_per_person_capacity.png", occ_rt_pl)
occ_rt_pl
```
...
adr_pc_pl <- ggplot(data=listings_df, mapping=aes(y=average_daily_rate, x=as.factor(person_capacity))) +
              ...
              labs(x='Room Type', fill='Person Capacity', y='Average Daily Rate')
...
top_performering_listings$occupancy = round(top_performering_listings$occupancy*100, digits = 1)
top_performering_listings$average_daily_rate = round(top_performering_listings$average_daily_rate, digits = 0)
top_performering_listings$yearly_revenue = round(top_performering_listings$yearly_revenue, digits = 0)
```

Amounts in EUR

```{r echo=FALSE, message=FALSE, warning=FALSE}
kable(top_performering_listings[ , c(1,11,5,2,4)], 
...
ggplot(data=top_performering_listings, mapping=aes(x=average_daily_rate,y=yearly_revenue)) +
      ...
      labs(x='Average Daily Rate (ADR)', y='Yearly Revenue in EUR', fill='Person Capacity', size='Revenue per available room (RevPar)', title='Top performing listings in Bali, Indonesia') 

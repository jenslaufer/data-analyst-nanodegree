...
axes_limits = c('only_message_from_poi_ratio',
                              'no_new_features', 'top_5_feature_list_from_kbest',
                              'top_10_feature_list_from_kbest', 'only_message_to_poi_ratio',
                              'all_new_features',  
                              'top_15_feature_list_from_kbest', 'only_total_financial_benefits', 'only_total_financial_benefits_with_messages'
                              )
metrics_df %>% 
  filter(tag!='pre') %>%
  filter(tag!='tuned') %>%
...
#### Algorithm quality without all new features

All new features were applied to final classifier

#### Algorithm quality with
...
But when the test set is applied it can be seen
that recall and precision are 0, so the model was actually not able 
to identify any true POI in the test set.

...
The goal of this project is to develop a machine learning model to classify, if a
...
and precision are touching the goal of 0.3 without optimization. But when the test set is applied it can be seen
that recall and precision are 0. The classifier fails on the test set as it could not indentify any POIs (true positives)
...
but it's ability of finding the negative samples is limited. It seems strange that the classifier performs
...
The unoptimized LogisticRegression fails completely on the test set as it cannot identify 
any POIs (true positives), which is reflected on  a recall and precision score of 0, although the accuracy seems
OK.
...
The performance of the classifiers with their default parameters
was not sufficient.  Therefore a parameter tuning is needed.
...
The whole data set is splitted into a trainig set and test set. The test is 20% of the data.
...
Pipeline(steps=[(‘reduce_dim’, PCA(copy=True, iterated_power=7, n_components=0.6, random_state=None,
svd_solver=‘auto’, tol=0.0, whiten=False

```{r warning=FALSE, echo=FALSE, message=FALSE}
summary <-   metrics_df %>% filter(tag=='tuned')
summary <- summary[ , !(names(summary) %in% c('tag', 'classifier'))]
```

```{r warning=FALSE, echo=FALSE, message=FALSE}
summary %>%
  kable(col.names=c('f1', 'accuracy','precision','recall', 'Dataset'))
```
